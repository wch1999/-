...
def storeAndParse_json(json_url_lists):  # 获取所有的json文件并且储存
    ...
    with open('各国疫情数据.csv', 'a', encoding='utf-8') as f_all: #创建一个名为各国疫情.csv的文件
        ...
        for json_url_list in json_url_lists:
            urlretrieve(json_url_list[2], filename=json_url_list[0] + "-" + json_url_list[1] + ".json")  # 0:大洲，1：国家，2：json的url

            #向csv写入该国家的信息
            with open(json_url_list[0] + "-" + json_url_list[1] + ".json", encoding='utf-8') as f:
                contents = f.read()
                infos = re.findall(r"""
